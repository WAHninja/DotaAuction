...
export default function AuctionPhase({
  ...
  const handleSubmitOffer = async () => {
    ...
    try {
      ...
      // Notify all clients via Ably
      await publishToMatchChannel(gameId, 'new-offer', { gameId });
  ...
  const handleAcceptOffer = async (offerId: number) => {
    ...
    try {
      ...
      // Notify all clients via Ably
      await publishToMatchChannel(gameId, 'offer-accepted', { gameId });
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {offers.map((offer) => {
          ...
          return (
            <div
              key={offer.id}
              className={`relative p-5 rounded-3xl shadow-2xl border transition-transform transform hover:scale-105 hover:shadow-2xl
