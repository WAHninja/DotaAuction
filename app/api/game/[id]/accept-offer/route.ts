...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  try {
    ...
    if (!offerId || isNaN(Number(offerId))) return new Response(JSON.stringify({ message: 'Invalid offer ID.' }), { status: 400 });
    ...
    await db.query('UPDATE match_players SET gold = gold + $1 WHERE user_id = $2 AND match_id = $3', [offer_amount, from_player_id, game.match_id]);
    await db.query(
    ...
    // Swap target player to other team for new game
    ...
    // ✅ Return accepted offer + new game — front-end listeners handle realtime
    return new Response(JSON.stringify({ acceptedOffer: offer, newGame: newGameRows[0] }), {
      status: 200,
